...
parser.add_argument("--result_dir1", default='out/bert/16_3e-5', type=str)
parser.add_argument("--result_dir2", default='out/bert/video_32x3_K400_3crop', type=str)
parser.add_argument("--model", default='bert', type=str)
# parser.add_argument("--batch_size", nargs='+', default=('4', '8', '16'), type=str)
...
parser.add_argument("--metric", default='averaged_f1', type=str)
...
def get_result(result_dir):
    ...
    for seed in args.seed:
        with open(os.path.join(result_dir, seed, f"results_test.json"), 'r') as f:
    ...
    # with open(os.path.join(result_dir, f'results_test.txt'), 'w') as f:
    #     for strategy in Strategies:
...
def main():
    ...
    alpha = 0.05
    data_A = get_result(args.result_dir1)
    print(data_A)
    data_B = get_result(args.result_dir2)
    print(data_B)
    ...
    if(name=="Shapiro-Wilk" or name=="Anderson-Darling" or name=="Kolmogorov-Smirnov"):
        ...
        if(float(output)>float(alpha)):
            answer = input("\nThe normal test is significant, would you like to perform a t-test for checking significance of difference between results? (Y or N) ")
            if(answer=='Y'):
                # two sided t-test
            ...
            answer = input("\nThe normal test is not significant, would you like to perform a non-parametric test for checking significance of difference between results? (Y or N) ")
            if (answer == 'Y'):
